...
$DeliverableObject = [ordered]@{
  ...
  properties = [ordered]@{
    riskScore = [ordered]@{
      ...
      minimum = 0
      maximum = 100
      description = 'Heuristic risk score from 0 to 100. Higher means more observed risk; this is not a probability.'
    }
    riskLevel = [ordered]@{
      type = 'string'
      enum = @('LOW','MEDIUM','HIGH','CRITICAL')
      description = 'Risk band derived from the heuristic score.'
    }
    honeypot = [ordered]@{
      type = 'boolean'
      description = 'GoPlus-reported honeypot status for this exact Base token.'
    }
    dangerousPermissions = [ordered]@{
      type = 'array'
      items = [ordered]@{ type = 'string' }
      description = 'Observed contract controls or behaviors that can increase risk.'
    }
    liquidityRisk = [ordered]@{
      type = 'string'
      enum = @('UNKNOWN','LOW','MEDIUM','HIGH','CRITICAL')
      description = 'Risk band for the largest matching Base pool liquidity.'
    }
    holderConcentration = [ordered]@{
      type = 'string'
      description = 'Concentration assessment from the reported non-burn, non-pool holder sample.'
    }
    tradingActivity = [ordered]@{
      type = 'string'
      description = '24-hour volume and buy/sell activity for the selected matching Base pool, or UNKNOWN.'
    }
    warnings = [ordered]@{
      type = 'array'
      items = [ordered]@{ type = 'string' }
      description = 'Explicit caveats, unknowns and source-quality warnings.'
    }
    summary = [ordered]@{
      type = 'string'
      description = 'Compact screening summary with chain, sources and engine version.'
    }
  }
  additionalProperties = $false
}
...
if (-not (Get-Command acp -ErrorAction SilentlyContinue)) {
  throw 'ACP CLI is not installed.'
}
...
if ($LASTEXITCODE -ne 0) {
  throw 'Could not select Jepeta Risk Guard.'
}
...
Write-Host '1/4 Upgrading offering copy and machine-readable schemas...'
...
if ($LASTEXITCODE -ne 0) {
  throw 'Could not list ACP offerings.'
}
$offer = $offerings | Where-Object { $_.id -eq $OfferingId } | Select-Object -First 1
if (-not $offer) {
  throw 'Token Risk Scan offering not found.'
}
...
if ($LASTEXITCODE -ne 0) {
  throw 'Could not inspect signer policy.'
}
...
if (-not ($policyFields -contains 'signerId') -or
    -not $policy.signerId -or
    -not ($policyFields -contains 'policyIds') -or
    @($policy.policyIds).Count -eq 0) {
...
foreach ($query in $queries) {
  try {
    ...
    $match = @($browse.data) |
      Where-Object { $_.walletAddress -and $_.walletAddress.ToLower() -eq $Provider } |
      Select-Object -First 1
    if ($match) {
      $foundBy += $query
    }
  } catch {
    # Discovery is advisory; listing validation above is authoritative.
  }
}
...
@{
  ...
  Set-Content -Path (Join-Path $StateDir 'discovery.json') -Encoding UTF8
...
if ($foundBy.Count -gt 0) {
  ...
  Write-Host 'Discovery indexing was not confirmed from this seller profile yet. The offering state is valid; recheck after marketplace indexing.'
}
