...
function scanFailure(error) {
  ...
  const code = typeof error?.code === 'string' ? error.code : 'SCAN_UNAVAILABLE';
  return {
    code,
    retryable: !(status >= 400 && status < 500),
    message: status >= 400 && status < 500
      ? 'This exact Base token is not safely covered by the required data sources.'
      : 'Required risk data is temporarily unavailable.',
  };
}

async function notifyBuyerOnce({ record, save, api, id, payload, key }) {
  if (!api?.message || record[key]?.attempted) return;
  record[key] = { attempted: true, at: new Date().toISOString() };
  await save();
  try {
    await api.message(id, JSON.stringify({
      type: 'jepeta_status',
      version: 1,
      offering: CONFIG.name,
      ...payload,
    }));
    record[key].sent = true;
    await save();
  } catch {
    record[key].sent = false;
    await save();
  }
}
...
export function inspectJob(job, history, now = Date.now()) {
  ...
  if (!/^0x[a-f0-9]{40}$/.test(addr(job.clientAddress)) || addr(job.clientAddress) === CONFIG.provider ||
      addr(job.evaluatorAddress) !== addr(job.clientAddress))
    throw new Error('Pilot requires a separate buyer as evaluator.');
  ...
  const events = systemEvents(history, id);
  const created = events.find(e => e.type === 'job.created');
  ...
  if (payloads.some(p => p.tokenAddress !== payloads[0].tokenAddress))
    throw new Error('Buyer changed requirements; operator review required.');

  const last = events.at(-1)?.type;
  const tokenAddress = payloads[0].tokenAddress;
  ...
  if (!Number.isFinite(expiry) || expiry - now < 45000)
    throw new Error('Not enough time to meet the job expiry.');
  ...
  if (funded && last === 'job.funded') {
    if (Number(budget?.amount) !== 0.03 || Number(funded.amount) !== 0.03 ||
        addr(funded.client) !== addr(job.clientAddress) || String(job.budget) !== CONFIG.priceRaw ||
        job.jobStatus !== 'FUNDED')
  ...
  if (job.jobStatus !== 'OPEN' || (job.budget && String(job.budget) !== '0'))
...
export async function processJob({ job, history, state, save, api, scanner, live = false, now = Date.now(), blockNumber }) {
  const decision = inspectJob(job, history, now);
  const { id, tokenAddress, action } = decision;
  const previous = state.jobs[id];

  if (previous?.submitAttempted && !previous.submitted) {
    const delivered = systemEvents(history, id).find(e => e.type === 'job.submitted' &&
      addr(e.provider) === CONFIG.provider && typeof e.deliverable === 'string' &&
      createHash('sha256').update(e.deliverable).digest('hex') === previous.reportHash);
    if (delivered) {
      previous.submitted = true;
      previous.submittedAt ??= iso(now);
      await save();
    }
  }

  if (action === 'terminal' && previous) {
    previous.terminal = true;
    previous.terminalAt ??= iso(now);
    await save();
  }
  ...
  if (action === 'quote') {
    if (record.preflightBlocked)
      return { id, action: 'unavailable', code: record.lastError?.code || 'SCAN_UNAVAILABLE' };
    if (record.quoteAttempted)
      return { id, action: 'needs_reconciliation' };

    if ((record.preflightAttempts ?? 0) >= CONFIG.maxPreflightScans) {
      record.preflightBlocked = true;
      await save();
      return { id, action: 'unavailable', code: record.lastError?.code || 'SCAN_UNAVAILABLE' };
    }

    record.preflightAttempts = (record.preflightAttempts ?? 0) + 1;
    record.lastActivityAt = iso(now);
    await save();

    let report;
    try {
      report = await scanner(tokenAddress);
      validateReport(report);
    } catch (error) {
      const failure = scanFailure(error);
      record.lastError = { stage: 'preflight', ...failure, at: iso(now) };
      if (!failure.retryable || record.preflightAttempts >= CONFIG.maxPreflightScans)
        record.preflightBlocked = true;
      await save();

      if (record.preflightBlocked) {
        await notifyBuyerOnce({
          record, save, api, id, key: 'preflightNotice',
          payload: {
            stage: 'preflight',
            status: 'unavailable',
            code: failure.code,
    ...
    record.reportAt = iso(now);
    record.startBlock = await blockNumber();
    record.quoteAttempted = true;
    await save();

    const result = await api.setBudget(id);
    if (result?.success !== true)
      throw new Error('Budget submission was not confirmed.');
    record.quoted = true;
    record.quotedAt = iso(now);
    record.lastActivityAt = iso(now);
    await save();
  ...
  let report = null;
  const reportAge = Date.parse(record.reportAt || '') ? now - Date.parse(record.reportAt) : Number.POSITIVE_INFINITY;
  if (reportAge >= 0 && reportAge <= CONFIG.reportCacheMs && record.cachedReport) {
    validateReport(record.cachedReport);
    ...
    try {
      ...
      if (exhausted) {
        await notifyBuyerOnce({
          record, save, api, id, key: 'fundedFailureNotice',
          payload: {
            stage: 'funded',
            ...
            retryable: failure.retryable,
            message: failure.message,
            action: 'Reject/refund this ACP job if no deliverable is available before expiry.',
          },
        });
      }
      return { id, action: exhausted ? 'delivery_unavailable' : 'retry_scan', code: failure.code };
    }
  }
  ...
  record.reportHash = createHash('sha256').update(deliverable).digest('hex');
  record.lastActivityAt = iso(now);
  await save();

  const result = await api.submit(id, deliverable);
  if (result?.success !== true)
    throw new Error('Delivery submission was not confirmed.');
  record.submitted = true;
  record.submittedAt = iso(now);
  record.lastActivityAt = iso(now);
  await save();
