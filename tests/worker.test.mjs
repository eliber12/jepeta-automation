...
test('local capacity blocks a second job without asking for payment',async()=>{const a=args(fixture());a.state.jobs['8']={quoted:true};let quotes=0,messages=0;a.api={...a.api,setBudget:async()=>{quotes++;return{success:true};},message:async(id,text)=>{messages++;const body=JSON.parse(text);assert.equal(body.status,'busy');assert.equal(body.paymentRequested,false);return{success:true};}};const r=await processJob(a);assert.equal(r.action,'busy');assert.equal(a.state.jobs['7'].capacityBlocked,true);assert.equal(quotes,0);assert.equal(messages,1);});
test('invalid report never quotes and is bounded',async()=>{const a=args(fixture());a.scanner=async()=>({});let calls=0;a.api.setBudget=async()=>{calls++;};assert.equal((await processJob(a)).action,'retry_preflight');assert.equal((await processJob(a)).action,'unavailable');assert.equal(a.state.jobs['7'].preflightAttempts,2);assert.equal(a.state.jobs['7'].preflightBlocked,true);assert.equal(calls,0);});

test('unsupported token fails closed, notifies once and never asks for payment',async()=>{const a=args(fixture());let scans=0,messages=0,quotes=0;const err=Object.assign(new Error('not covered'),{status:422,code:'TOKEN_NOT_COVERED'});a.scanner=async()=>{scans++;throw err;};a.api={...a.api,setBudget:async()=>{quotes++;return{success:true};},message:async(id,text)=>{messages++;const body=JSON.parse(text);assert.equal(id,'7');assert.equal(body.status,'unavailable');assert.equal(body.paymentRequested,false);return{success:true};}};assert.equal((await processJob(a)).action,'unavailable');assert.equal((await processJob(a)).action,'unavailable');assert.equal(scans,1);assert.equal(messages,1);assert.equal(quotes,0);});
...
test('read-only settlement end-to-end verifies two confirmations',async()=>{const rpc=async method=>({'eth_chainId':'0x2105','eth_blockNumber':'0x104','eth_getLogs':[{transactionHash:'0x'+'a'.repeat(64)}],'eth_getTransactionReceipt':receipt()}[method]);const proof=await verifySettlement('7','0x100',rpc);assert.equal(proof.creditedUSDCraw,'27000');assert.equal(proof.verificationType,'onchain_acp_usdc');});
